...
class AddGames extends React.Component {
    ...
    formValidation = () => {
        ...
        if (!gameObj.name) {
            $("#gameName").siblings(".helper-text").text("`").attr("data-error", "Name is required");
            $("#gameName").addClass("invalid");
            badForm = true;
        } else {
            helpName.siblings(".helper-text").attr("data-error", "").text("");
            helpName.removeClass("invalid");
        }

        if (!gameObj.minPlayers) {
            helpMinPlayers.siblings(".helper-text").text("`").attr("data-error", "Min Players is required");
            helpMinPlayers.addClass("invalid");
            badForm = true;
        } else {
            helpMinPlayers.siblings(".helper-text").attr("data-error", "").text("");
            helpMinPlayers.removeClass("invalid");
        }

        if (gameObj.maxPlayers) {
            if (Number(gameObj.maxPlayers) < Number(gameObj.minPlayers)) {
                helpMaxPlayers.siblings(".helper-text").text("`").attr("data-error", "Max Players < Min Players");
                helpMaxPlayers.addClass("invalid");
                badForm = true
            } else {
                helpMaxPlayers.siblings(".helper-text").attr("data-error", "").text("");
                helpMaxPlayers.removeClass("invalid");
            }
        }

        if (!gameObj.minPlaytime) {
            helpMinPlaytime.siblings(".helper-text").text("`").attr("data-error", "Min Playtime is required");
            helpMinPlaytime.addClass("invalid");
            badForm = true;
        } else {
            helpMinPlaytime.siblings(".helper-text").attr("data-error", "").text("");
            helpMinPlaytime.removeClass("invalid");
        }

        if (gameObj.maxPlaytime) {
            if (Number(gameObj.maxPlaytime) < Number(gameObj.minPlaytime)) {
                helpMaxPlaytime.siblings(".helper-text").text("`").attr("data-error", "Max Playtime < Min Playtime");
                helpMaxPlaytime.addClass("invalid");
                badForm = true;
            } else {
                helpMaxPlaytime.siblings(".helper-text").attr("data-error", "").text("");
                helpMaxPlaytime.removeClass("invalid");
            }
        }

        if (!gameObj.minAge) {
            helpAge.siblings(".helper-text").text("`").attr("data-error", "Min Age is required");
            helpAge.addClass("invalid");
            badForm = true;
        } else {
            helpAge.siblings(".helper-text").attr("data-error", "").text("");
            helpAge.removeClass("invalid");
        }

        if (!gameObj.rating) {
            helpRating.siblings(".helper-text").text("`").attr("data-error", "Rating is required");
            helpRating.addClass("invalid");
            badForm = true;
        } else {
            helpRating.siblings(".helper-text").attr("data-error", "").text("");
        ...
        if (this.formValidation()) {
            ...
            gameLogic.saveGame(gameObj).then(resp => {
                ...
                this.setState({
                    ...
                    image: "",
                    complexity: ""
                }, () => { M.updateTextFields(); $(".yuppertupper").removeClass('valid'); M.toast({ html: '<span>Game added!</span><a href="/mylibrary"><button class="btn-flat toast-action">Go to Library</button></a>' }); });
            });
        }
    };
